...
- `LibraryStore` — household capabilities, share matching, the S1/S2 availability tags (a share is per-system, not per-path), and `browseSections`. Depends on `ContentDirectoryBrowsing`; media-server sections arrive through `BrowseSectionContributing` so the store knows nothing about them.

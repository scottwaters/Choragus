...
A feature release. Choragus browses and plays from UPnP/DLNA media servers, builds playlists from a written brief through a chat AI, and runs a Model Context Protocol server so an assistant can drive the speakers; alarms get a window, the queue repairs and monitors itself, Amazon Music and Pandora connect, Plex playback reports back to the server, and a Select Input action joins Shortcuts. Build 35.

### Media servers (DLNA / UPnP)

- Choragus browses and plays from UPnP/DLNA media servers (Plex, Synology Media Server, MinimServer and the like) even though a Sonos speaker cannot: the server supplies the browse tree and the audio URL, the speaker plays the URL. Servers are discovered by SSDP or added by address in Settings → Media Servers; each is probed for its ContentDirectory service and browsed into the sidebar under its own name.
- Server-published artwork is a first-class artwork source, consulted before the resolved/unresolved split; queue rows fall back to the shared iTunes memo when a speaker proxy 404s. Media-server roots are pruned by a first-page class verdict so video-only and image-only subtrees stay out of a music browser.
- A per-speaker reachability check makes each speaker fetch a reference track through its own `/getaa` proxy, so a server on a VLAN one speaker cannot route to is named, with the address it fails to reach, instead of presenting as a silent queue. Runs automatically when a server is added by address; Help covers the firewall rule.
- Media-server search backs the Playlist Builder through the ContentDirectory `Search` action, fail-open on servers without search capability.
- A media server can be given a display name of your own: an optional field under each server in Settings → Music → Media Servers. The title is applied where the manager publishes its list, so the sidebar, Browse, the queue, Diagnostics and the agent server all show the one name; the advertised name stays on the record, is shown beside the field, and comes back when the field is cleared. `list_media_servers` reports `advertised_name` for a renamed server, and the other media-server tools accept either name.
- The Media Servers section is laid out as one card per server: the name on one line (long names truncate rather than wrap), host and advertised name on a caption line beneath, the reachability sentence, then a labelled Display name field, with a divider and a labelled Add a server field below the list so the two fields cannot be mistaken for each other.
- Security bounds on everything a LAN peer can send: description URLs restricted to web schemes and private addresses, 256 KB caps on device and server descriptions, 2 MB on SOAP responses, SSDP `LOCATION` hosts must match the datagram sender, the mDNS TXT location shortcut is gone, and the event listener accepts `NOTIFY` only from discovered speakers.

### Build Playlist with AI

- The brief field is three lines tall and grows with the text. It shows a different sample prompt each time the window opens, twelve in all across electronica, rock, folk, jazz, dubstep, classical, pop and country, in all 13 languages; a Sample prompts link under the field opens the Help page that lists them all.
- Saving a Choragus playlist from the builder offers a folder picker, nested folders indented, so a playlist can land in a Queue Library folder directly.
- The builder's bottom bar is headed "Send to" with the destination in one picker — Choragus playlist, Add to Queue, Play Next, Play Now. The three queue destinations show a Room picker listing the current rooms and groups, defaulting to the one selected in the main window, instead of sending to the last-selected room silently. The Playlist Manager link after a delivery appears only when a playlist was saved.
- The Send to button dims while it cannot act: no songs matched yet, a match still running, or a Choragus playlist with no name. It kept its full accent colour in those states and read as live.
- Browse → Choragus Sources → Build Playlist describes a playlist to a chat AI and turns the reply into real, playable tracks. Songs stream into a table as the model writes them; Match to <service> resolves them on Apple Music, any authenticated SMAPI service, the Sonos local library or a DLNA server (artist match mandatory — title-only matching returned cover versions; hosted lookups pace at 2 s per song, LAN lookups at 150 ms); the result goes to a saved Choragus playlist, the end of the queue, play next or play now, without re-matching. A copy-paste prompt template serves any external AI when no service is configured.
- Settings → AI holds a list of named AI services — Claude, OpenAI, or any OpenAI-compatible endpoint (DeepSeek, Ollama, LM Studio) — each with its own model, endpoint and keychain key, with add, remove, reorder and a connection test. A bare custom host gains `/v1`, the effective URL shows under the field, and a server that answers an unknown path with a 200 JSON error is reported as that error rather than as an empty reply.
- The AI reply is untrusted input end to end: typed decode, control-character stripping, field and list caps, byte-wise SSE parsing with size caps and mid-stream error surfacing; keys are sent only to their own provider's host and refused over cleartext to non-local addresses.
- Help → AI Playlists explains the flow, with sample briefs and the paste format, in all 13 languages.
- **docs/AI.md**, a guide to the four levels of AI: none, copy-and-paste playlists, connected playlist building, an assistant driving Choragus over MCP, and the same from a phone through a session running on the Mac. Flowcharts per level, setup steps, what leaves the Mac at each level, and troubleshooting. The README's v5 section and docs/MCP.md link into it.
- Settings → AI is two cards: **AI playlist generation** holds the enable switch, the service list and the selected service's connection settings together, and **AI Agent access (MCP)** holds the server. Settings → AI fills the model picker from the provider: the Claude and OpenAI model lists are fetched with the profile's key (OpenAI reduced to chat families, dated snapshots dropped), with a reload button. Without a key, or when the fetch fails, the built-in list stands and the reason shows under the picker.

### AI Agent access (MCP)

- Choragus is a Model Context Protocol server (Streamable HTTP, JSON-RPC 2.0, `POST /mcp`) behind Settings → AI → AI Agent access: off by default, loopback-only unless the user allows the LAN, named bearer tokens (one per assistant, add / copy / revoke, last-used stamp) generated on the Mac and kept in the keychain, constant-time compared on every request, requests capped at 1 MB. Bonjour `_mcp._tcp` advert on LAN mode so Wake for network access can wake the Mac.
- 116 tools over `SonosManager`, one file per area: rooms, devices, now playing, transport (incl. stop, pause_all, resume_all), room / group-proportional / per-speaker / relative volume, mute, play mode (shuffle, repeat, crossfade), sleep timer, per-speaker EQ (bass, treble, loudness, night mode, speech enhancement), grouping incl. group_all / ungroup_all, presets (list, apply, save, delete), line-in inputs, rescan; library search with a `type` filter (tracks, artists, album artists, albums, genres, composers, playlists), `browse_library` over the section tree, library shares and reindex, DLNA media servers (list, browse, search), TuneIn radio search, Sonos favorites, signed-in service search (Apple Music and any SMAPI service); queue read, batch add, jump, move, remove, dedupe, clear, save as playlist; Choragus playlists (tracks, create from results, add, remove, rename, duplicate, delete, folders, move); play history (recently played, most played over N days, starred, star, smart queues). Results carry ids from a 500-entry cache that every play / queue / playlist tool accepts. Tool descriptors carry `readOnlyHint` and `destructiveHint` annotations (destructive = takes `confirm`).
- `search_service` asks each service in its own search vocabulary. The ids are per service: most answer to the generic `track` / `album` / `artist`, but Amazon Music publishes `catalog:tracks:search`, `catalog:albums:search`, `catalog:artists:search` and `catalog:universal:search`, and answers a generic id with an empty result rather than a fault — so every Amazon search through an assistant came back as "nothing found". The tool now resolves the service's published categories (the same discovery the Browse window runs, through one shared `SMAPISearchCategories` cache), tries the category for the requested kind, then the service's search-everything category, then the generic id, and stops at the first that answers. A service that does not answer at all is reported as a failed search instead of an empty one.
- A service search category that the service lists but never fills is dropped from the search chips. Amazon Music publishes a Tracks category and answers it with an empty list on an account whose Albums, Artists and Everything categories match the same term; the Sonos app offers no track search for it either. The "All" search sends one term to every category, so it is the evidence: a category that returned nothing while another returned five or more rows is hidden, re-checked on every "All" search, and back the moment it answers.
- Playlist builder as a background job: `build_playlist` returns a `job_id` at once and matches on the library (default), Apple Music, a signed-in service or a media server, with an optional `fallback_service` that re-tries only the misses; `deliver: queue` appends each match to the room's queue as it lands, `deliver: play` starts the first match and queues the rest, `save` stores the result as a Choragus playlist at the end. `build_status` (with `wait_seconds` long-poll) and `cancel_build` follow the MCP Tasks extension vocabulary (`working`, `completed`, `failed`, `cancelled`, `poll_interval_ms`). `generate_song_list` runs the selected AI profile from Settings; `list_ai_profiles` reads them. `MCPBuildJobs` knows nothing about Sonos — the tool layer supplies the queue and save closures — and is unit-tested with a fake matching service (streaming order, fallback pass, cancel, hit errors, finish failure).
- Security: per-token access levels (read only / control / manage) set at creation and editable in Settings, enforced per tool with the reason returned in the result (legacy tokens decode as manage so configured assistants keep working); `Origin` and `Host` validation refuses web pages and non-local host names (private addresses, `.local` and bare names pass on the LAN); five wrong tokens from one address in a minute lock it out for five minutes with `Retry-After`; 60 requests per 10 s per token; a **Volume limit for agents** (default 80) caps every volume tool; the peer address travels on `HTTPRequest` for the guard and the log. `MCPAccessGuard` is unit-tested with an injected clock.
- Monitoring: `MCPActivityLog` records every request (token, action, argument summary, outcome, duration, address) into a 300-entry ring shown as **Recent activity** in Settings with per-token call counts, active lockouts and a Clear button; each entry is also a `[MCP]` diagnostics line for bug reports.
- Review pass: `list_presets` returns each preset's members, volumes and EQ; `list_inputs` marks the `primary` unit of a stereo pair and its channel; `list_rooms` carries the coordinator room and current title/artist; `seek` takes `delta_seconds` and clamps to the track; `group_rooms` takes a `rooms` array, resolved before any join; `group_all` snapshots the grouping and returns `snapshot_id` for `restore_grouping` (new, with `snapshot_grouping`) so party mode can be undone without splitting pre-existing groups; `activate_preset` caps members at the agent volume limit; every playlist tool accepts `playlist` (name or id) as well as `playlist_id`; playlist names are unique for agents (`replace: true` overwrites, renames and copies refuse a used name); `build_playlist` fails at once on a used name, names the missing `room`, and takes `playlist` to append fallback matches to the original; `get_queue` and `get_playlist_tracks` page with `start`; positional queue and playlist edits take `expected_total` and refuse to run on a changed collection; `destructiveHint` is an explicit per-tool flag (removals, deletes, `ungroup_all`, `reindex_library`), `idempotentHint` marks reads, and the main read tools declare `outputSchema`; descriptions front-load the tool name and synonyms so tool search finds them.
- The main toolbar carries an AI Agent access indicator while the server is on: an **MCP** pill with a light in the server's state (green running, orange failed, grey stopped) and the port and session request count in its tooltip; clicking it opens Diagnostics on the AI Agent access tab. It sits beside Settings and can be hidden with **Show MCP status in the toolbar**.
- Per-token **Skip confirmations** (Settings → AI Agent access): the `confirm` flag is not required from that token; the destructive annotations stay so a client can still ask. Off by default.
- Allow other devices fixed: the LAN listener came up, then died silently when its attached Bonjour advert failed, and nothing logged it — the state handler ignored failures after the first ready. Bonjour is now published separately (`NetService`), every listener state change after start is logged as `[MCP] Listener …`, and a failure sets the Settings status. Verified: `*:52080` bound, answers on every interface address.
- Third review pass: the loopback listener binds to `127.0.0.1` itself (was a wildcard socket restricted to the loopback interface); `get_queue` returns the speaker's queue `revision` (Q:0 UpdateID) and positional edits take `expected_revision`, which catches reorders `expected_total` cannot; `remove_from_queue` documents snapshot semantics and returns `remaining` plus the new revision (the old `total` field was wrong when `expected_total` was absent); every tool takes `request_id` and a repeated mutating call returns the first result instead of acting twice; `oneOf` on `seek`, `group_rooms` and every playlist selector; result text capped at 200 characters; **Quiet hours** (Settings → AI Agent access) apply a lower volume cap between two hours; playlist builds keep input order across fallback passes (regression-tested with alternating hits), run at most two at a time, record `queued_positions`, and can be undone (`cancel_build` `undo`, `undo_build`); the LAN toggle's help says the token travels unencrypted.
- `listening_stats` exposes the Listening Stats window's numbers (hours, daily series, hour-of-day, source and room distributions, streak). Queue rows and playlist tracks without a length — every local-library track, since Sonos reports none — take the length play history recorded when the track last played (`QueueDurationSource`, matched by URI, then title/artist/album); the Queue panel total, Queue Library and MCP `get_queue` / `get_playlist_tracks` all pick it up.
- Diagnostics → AI Agent access tab: a banner with server state, tokens and lockouts, then the request log as a table (time, token, action, outcome, duration, arguments) with a failures filter; selecting a row loads its full request and response from disk into a detail pane with copy buttons. Payloads live in `Application Support/Choragus/mcp-payloads/` (`MCPPayloadStore`: one file per call, newest 300 / 20 MB kept, written off the main thread, never held in memory). The bug-report bundle's `mcp` section carries the server state, tokens (names and scopes only), lockouts, running builds and the request log, with request and response for the newest 100 calls trimmed to 8 KB each.
- Alarms: `list_alarms`, `create_alarm` (chime or any result id as the program), `set_alarm`, `delete_alarm` (`confirm`). Under them, alarm reads and writes now go to the speaker that owns the household's alarm list (named by `CurrentAlarmListVersion`) instead of whichever coordinator answered first; a list read from another player within seconds of a create or delete was stale, which showed in the Alarms window as an alarm that would not go away. `AlarmClockService.listAlarmList` returns the master id alongside the alarms.
- Three resources (`choragus://rooms`, `choragus://now-playing`, `choragus://playlists`) and three prompts (`play_for_me`, `playlist_from_brief`, `party_mode`) that spell out the tool sequence for a client.
- Companions in the same Settings section: Prevent the Mac from sleeping (an IOKit power assertion held while the server is on) and Open Choragus at login (`SMAppService`). Help → AI Playlists and docs/MCP.md cover client setup for Claude Code, Claude Desktop (`mcp-remote`), OpenAI Codex, the OpenAI Agents SDK and Responses API, Cursor, VS Code, the Python `mcp` package, and the raw JSON-RPC protocol.
- `LocalHTTPServer` (Network.framework, one request per connection, 16 KB header / 1 MB body caps) and the JSON-RPC dispatcher are unit-tested; `SMAPIAuthManager.current` joins `SonosManager.current` for kit-side callers.

### Queue

- A bare Apple Music queue row next to the playing track is repaired once playback moves on. The follow-up that names Apple Music rows added bare (re-enqueue with the service descriptor, drop the bare row) has to leave the playing row and the one after it alone; those were dropped from the repair and stayed untitled with no length for good, and Now Playing showed "Live" when the track came round. Deferred rows are retried after a delay, each pass re-checking the row is unchanged and still bare, and the playing track's length is filled from play history the way queue rows already are.
- The repair retries the moment the playing track number changes, and its timer ceiling is long enough to outlast any track; a 4:11 song had outlived the first budget by half a minute.
- Every track change also sweeps the queue for Apple Music rows the speaker wrote bare and schedules the same repair for them, so rows deferred before a relaunch, or present before the repair existed, are still picked up.
- The catalogue lookup that names an Apple Music track now records its length too, so a bare row that is already playing shows its time instead of "Live" once the lookup lands; a lookup refused by the rate limiter is retried a few times while the same track plays. That lookup draws on two slots of the per-minute iTunes budget that artwork, browse and history backfill cannot take, so a queue-wide artwork re-pin no longer holds it off for the whole play.

- Multi-select: click selects, ⌘-click toggles, ⇧-click extends, double-click plays, Delete removes. The context menu acts on the selection — Play, Move to Top / Bottom, Add to Choragus Queue (new or existing), Remove — and dragging a selected row moves the whole block, order preserved, with one reload at the end.
- The header shows the queue's estimated running time (`N tracks · h:mm:ss`, `~` when rows lack a duration or pages are still loading); the Queue Library detail shows the same for a playlist.
- Automatic health monitoring: after a track change the queue's next rows are checked for expired signed URLs, dead media-server hosts and rows enqueued without metadata, budgeted so steady state costs nothing (offline expiry checks are free; at most ten probes per pass, one TCP touch per host, verdicts cached, ten-minute cooldown per group). Unplayable rows grey out with a badge that explains itself; a strip offers one-click removal; rows whose origin is known are repaired in place with a fresh URL.
- Playback recovers from expired service URLs (TIDAL, Qobuz, Suno): the expiry is read from the URL, the originating service item is remembered so the URL can be re-resolved, saved queues carry the origin so they survive independently of that registry, and both restore and the early-advance detector repair expired entries before reporting anything.
- One batch add per coordinator at a time: overlapping adds from repeat clicks are refused instead of tripling the queue and saturating the speaker.
- The playing-row highlight survives the transitional metadata at track advance, click-to-play no longer flaps the indicator to row 1, and `CurrentTrack` is read from AVTransport events so the speaker's own position drives the highlight rather than a title match.
- Long titles slide to reveal themselves on hover instead of a popover.
- A footer under the queue shows the playing position (`Track 12 of 34`) and the time left of the total (`~1:23:45 remaining of 2:10:00`): the rest of the current track plus every row after it, `~` when rows lack a duration or pages are still loading.
- Every Choragus playlist menu — Add to Choragus Queue in the browse, queue and Plex context menus, and the queue panel's load menu — nests playlists as in the Queue Library sidebar: a Choragus group for playlists in no folder, then one submenu per folder, folders inside folders. The load menu adds a History submenu, one entry per room, with Replace and Append.

### Queue Library

- Artwork-grid cards line up across each row. The source and track count now sit above the name, so the name — the only line whose height varies — comes last, and the grid cells are top-aligned; a one-line name no longer drops its card a few points below its neighbours.

- Restoring a queue snapshot keeps the track details. A queue read preserves the speaker's `<r:resMD>` envelope when it sends one, and for many rows it sends none; those rows were re-enqueued as a bare URI, so the speaker wrote a queue row with no title, artist or album. The track played and the panel looked right until the next launch, because the titles were only in an in-memory cache — and the next snapshot of that queue inherited the loss, compounding it each round. The restore now rebuilds the missing envelope through the same builder the play-history replay uses, which reconstructs the service item id and descriptor from the URI. A generic envelope is not enough: a Spotify row given one carrying `<res>` and display text is discarded whole by the speaker.

- Deleting a Choragus playlist moves it to Deleted Items in the sidebar instead of removing it; Restore puts it back in its folders. Deleted playlists are removed for good after 30 days, or at once with Empty Deleted Items (confirmed first).
- Opening the window no longer blocks the app for about 3 s. The Most Played ranking searched its first-seen list inside the sort comparator, which is quadratic and cost about 700 ms per pass against a 13,000-row history, and the pass ran four times per open. The position is now looked up, and smart-queue covers reuse the ranked tracks instead of ranking again: 1.4 s per card build became 25 ms.
- List view: a click anywhere on the row opens it, and the row stays highlighted in the theme accent while its panel is open. The drag source is the grip and thumbnail at the front of the row; the grip appears on queues that can be dragged into a folder. Sidebar and row highlights read the theme accent directly instead of the system accent.
- List view columns sort: click a header to sort on it, click again to flip the direction; the active column shows a chevron.
- Selecting a queue no longer freezes the window for 100 to 300 ms before its panel appears. Cover cells drew full-size JPEGs that were re-decoded on every relayout; an A/B with flat colour in their place showed the images were the whole cost. Cover cells now ask the image cache for a decoded 240 px thumbnail, cached separately, and the cache's disk tier is read off the main thread instead of from `body`. Four window resizes that each stalled 70 to 170 ms now stall not at all. The memory tier grows to 800 items / 96 MB so a full grid of thumbnails stays resident.
- Right-click on a track in a queue's panel offers Play Now, Play Next and Add to Queue for the window's target room, on every queue kind; the reorder and remove actions stay on Choragus queues. `QueueItem.browseItem(id:)` replaces three hand-rolled copies of the same conversion.
- The debug log records per-open timings for the local card build, the Sonos browse and the cover pass.
- Sort by name or newest; Choragus playlists carry a created date. A year badge on rows where the release year is known.

### Browse

- One sort control on every browse and search list (local library, Sonos favourites, every SMAPI service, Apple Music via Sonos, TuneIn, Calm Radio, Sonos Radio). It offers relevance and title A–Z and Z–A always, artist A–Z and Z–A when a row names an artist, and newest and oldest when a row carries a release date, so a list never offers an order it cannot apply. Sorting is stable and case-insensitive; artist order breaks ties on title.
- Station rows never show a drill-down chevron, whatever stream scheme they use; tapping one plays it.
- One back bar and one bulk-action bar across the browse lists: the local library, Sonos favourites, SMAPI services, Apple Music via Sonos, TuneIn, Calm Radio and Plex direct share the same components, the item count under the bar is localised, and the bar disables while an action is in flight so a second click cannot enqueue a list twice.
- One vocabulary in every right-click menu: Play Now, Play Next, Add to Queue, Replace Queue and Browse. "Play All Now", "Play All Next", "Show Tracks" and "Open" are gone from the menus. Radio streams offer Play Now only, in every list, since a stream cannot sit in the queue.
- The queue's selection clears on a click anywhere else in the window or on Escape.
- Play and enqueue failures in service views and the Suno link error are localised.
- Calm Radio has the same chrome as every other service: Browse and Search tabs, genres as a level of rows that open into their channels, and a search across every channel of every genre. The Browse and Search tab labels on TuneIn and the SMAPI services are localised.
- Browse highlights follow the theme accent: the sidebar row hover and press tint, search filter chips, Calm Radio category chips and the A–Z index popover drew grey or with the system accent even when a theme colour was set. The queue's current-track and selected-row shading, drop indicator and filter toggle follow it too. `SonosManager.themeAccent` is the one accessor for highlight colours.
- The sidebar's categories are shaded cards with a headline title and expand chevron, collapsed by default, and can be dragged into any order. Rows highlight on hover.
- TuneIn in Choragus Sources runs on RadioTime's public API and resolves stations to a direct stream when the household no longer lists the legacy service; TuneIn (new) is a standard service row.
- Suno playlists resolve again through the playlist API (the page no longer renders per-track links) and can be added to the queue as well as played.

### Plex

- Direct Plex playback reports back to the server: the room shows as an active stream on the Plex dashboard, and a track that plays at least 90 % counts as a play (play count, last played, history), so smart playlists that depend on play counts work. Resume after stop and skips behave as in Plex's own clients (#86).
- Artists, albums, playlists and playlist items page through the whole listing; the artist level was one capped page (#85).
- "Plex – Cloud" is now "Plex – Remote", with a note under its Music Services row: Plex's Sonos service lists a server only when Remote Access has a direct public address, so a relay-only (CGNAT) server shows as unavailable. Plex – Local is unaffected.

### Now Playing

- Clicking the artwork opens the same carousel as the About thumbnails — artist photos plus the artwork on screen — opening on the tapped image.
- Artwork is stored as it arrives, and framed where it is drawn. Images fetched for the art cache used to be centre-cropped to a square first, and that crop measured its rectangle in `NSImage` points while applying it to the pixel-space `CGImage` — the two agree only at 72 DPI, so a 639x640 cover tagged 300 DPI kept its top-left 154x154 pixels and the view scaled that fragment up to fill the frame. Squaring was wrong even done correctly: a press photo is a tall portrait, the face is in the top third, and the centre square is the midriff, which is what the artist thumbnails in About were showing. The crop was also what reached the disk, so both survived relaunches. Cropping now happens only at display time, where each surface picks its own framing, and the artist thumbnails and the enlarged viewer keep the face.
- `CachedAsyncImage` takes a content mode and a fill alignment instead of a fill-to-frame flag: automatic still shows square art whole and fills with non-square art, the karaoke backdrop and the Queue Library cover force fill, the enlarged viewer forces fit, and the artist thumbnails anchor their crop to the top.
- The art cache purges itself once on first launch of a build carrying these fixes, since a stored file gives no sign that it was written by the affected code.
- The app theme is owned by `NSApp.appearance`, so a window set to System no longer inherits another window's Light or Dark.
- Scroll-wheel volume follows the system's Natural scrolling setting instead of assuming it (#87).
- A sustained media-key burst can no longer unlock playback on a locked Mac: presses are folded against the last event seen, not the last counted one.

### Alarms

- An alarm icon in the main toolbar opens a new Alarms window: each alarm shows its time, the days it fires as a week strip, room, music and volume, with a switch to enable it. Add, edit and delete are inline; the editor covers on/off, time, room, music (the Sonos chime or any Sonos favorite), a per-day schedule or once only, volume, duration or no limit, include grouped rooms, and shuffle — the same fields as the Sonos desktop app.
- Alarm reads and writes go to the speaker that owns the household's alarm list (`CurrentAlarmListVersion`); a list read from another player within seconds of an edit was stale, which showed as alarms that would not delete or appeared late. `SonosAlarm` now carries `playMode` (shuffle), parses `ON_135`-style day lists correctly (day numbers, not a bitmask), treats an empty duration as no limit, and exposes the program title from its DIDL.
- An alarm set to a favorite is stored with its title. The favorite's metadata reaches the alarm write still entity-escaped from Browse and the SOAP layer escaped it again, so the speaker kept it as text and every later read showed the raw `x-rincon-cpcontainer:` URI in place of the name. `AlarmClockService` normalises the DIDL on the way out and again on the way in, so alarms written by earlier builds show their title too.

### Shortcuts

- New Select Input action: plays a speaker's analog line-in or TV input in a chosen room or group. The Input picker lists every speaker with a physical input, read from the live system; the input and the playing room can differ. Selecting an input that is already playing leaves it playing, so the action is safe to run from a launchd agent or a Shortcuts automation (#84).
- Input discovery, stream URI and DIDL moved from the Browse Line-In view into `PhysicalInput` (SonosKit), shared by the view, the intent and `SonosManager.playInput`. Covered by `PhysicalInputTests`.
- `docs/SHORTCUTS.md` documents every action, the Siri phrases and scheduled runs; Help gains an Apple Shortcuts topic in all 13 languages, and README links both.
- Every intent string is localised: titles, descriptions, parameter names, summaries, entity names and Siri phrases through `Localizable.xcstrings` / `AppShortcuts.xcstrings` (system locale, as App Intents requires), and the not-found errors, input labels and fallbacks through `L10n` (in-app language). The bundle now declares all 13 languages, so standard AppKit menu and panel strings follow the system language too.

### Settings

- Tabs: Display, Music, Scrobbling, AI, Visualisations, System, Software Updates. Every tab uses the same section cards; the Scrobbling tab shows the connected Last.fm user beside its toggle.
- Media Servers: enable, discovered or manual mode, add and remove by address, reachability results per speaker.
- Advanced network options sit behind one row.
- Text fields with a character cap show a counter and stop accepting input at the cap.

### Services and identity

- Unrelated services no longer inherit another service's identity: the host-token fallback that folded 37 services into Sonos Radio (and RadioApp and Klassik Radio into Pandora) rejects generic tokens — sonos, radio, music, player, stream, media, audio. Radio Paradise and SomaFM are named in the service row.
- Durations over an hour read as hours everywhere: one `PlaybackTimeFormat` replaces four drifted copies.
- Amazon Music connects and plays (#88, patch by cdietze). It had been on the blocked list since v3.51 on a "returns empty auth URL" note that does not reproduce: `getAppLink` answers HTTP 200 with a usable `regUrl`, verified on his S1 and on an S2. Play rules are per Sonos generation. S1 (his verified form): SMAPI ids (`catalog:track:asin:<ASIN>`) are rewritten to the catalogue path the play URIs use, tracks play as `x-sonosapi-hls-static:` with `flags=0`, albums enqueue track by track. S2 (live tests on an AU household): tracks keep the verbatim id and play from the queue, verified with an Unlimited account; a station id the service supplies (`sp:container:station:…`, how albums and playlists arrive on an Amazon Music Prime account) plays as-is on the transport with the anonymous descriptor, and an artist row plays as its artist station the same way — both exactly what the Sonos app sets. Stations offer Play Now only. Stations play as `x-sonosapi-radio:` with `flags=8300` on both. Search ids carry a tracking fragment (`#erefid-…`) that is dropped before use. Nothing goes through `getMediaURI`, whose signed HLS manifest the speaker enqueues but faults 701 on.
- Amazon Music Prime, the tier bundled with Prime, is station playback only: a single track enqueues and resolves, then the speaker faults 701 on Play in every form. That refusal is reported as such ("single tracks need Amazon Music Unlimited") instead of a stale-topology rescan. Account serial discovery also reads the tracks each group is playing now, so a service linked in the Sonos app minutes ago is usable without a favourite.
- Amazon tracks classify as tracks, not radio (`URIPrefix.isHLSStaticTrack`), with their own art-cache source family so Apple Music `/getaa?` entries never leak onto them. Service-track DIDL now carries `dc:creator`, `upnp:album` and `upnp:albumArtURI`.
- Web-search art is a guess and yields to real art: a track that starts with title, artist and album but no art used to pin the first iTunes hit for the rest of the play (Beck "Mellow Gold" showed Rod Stewart "Gold"). Web pins now step aside when the speaker publishes art, failed `/getaa?` probes expire after 20 s, the art cache is consulted before any search, and an in-flight search cannot overwrite a cover that has since arrived.
- Skip buttons follow the speaker's `CurrentTransportActions` instead of a URI-scheme guess: an Amazon station allows Next but not Previous, a TuneIn stream neither, queue playback both. Read from every AVTransport event and once per poll.
- Pandora leaves the blocked list: `getAppLink` answers HTTP 200 with a link code (probed 2026-09-08 from an AU household). The row appears for households whose descriptor list carries Pandora; playback rules follow the Sonos app's station form and are untested.

### Events and discovery

- The event listener waits for its callback socket to release before rebinding on a network change, and falls back to an ephemeral port when the fixed port is still held — subscriptions no longer look healthy while every `NOTIFY` is dropped and the UI runs on the reconciliation poll.
- SonosManager migrated to `@Observable` and split into collaborators (TopologyStore, VolumeController, LibraryStore, TrackMetadataEnricher, QueueController) with typed protocols; views re-render only for the properties they read. Equality gates on the transport-state writers keep event and poll ticks from dirtying state that has not changed.

### Diagnostics, tests and CI

- Agent token secrets are drawn from the system's random number generator through `SystemRandomNumberGenerator`; the previous call discarded the status of `SecRandomCopyBytes`, so a failed read would have issued a token of zero bytes.
- The bug-report bundle's AI Agent access section passes through the same redaction as the log entries: peer addresses, lockout addresses and the recorded request and response bodies are scrubbed of LAN addresses, speaker ids and household ids before encryption, since `list_devices` and `list_rooms` results carry all three. Household ids (`Sonos_…`) are masked to their last four characters wherever the redactor runs.
- Diagnostic categories for media servers, the queue library, catalog folding, Plex sessions and volume verification; `docs/DIAGNOSTICS.md` explains each.
- Release builds finish in minutes rather than the better part of two hours. Translations moved out of Swift source into an Apple String Catalog (`Localizable.xcstrings`, a SonosKit resource); the 22,000-line dictionary literal it replaces had the Swift 6.3 optimiser's `cowarray-opt` pass running a cold-block analysis for about a minute per chunk per architecture, single-threaded. SonosKit now compiles at `-O` in about a minute.
- 1011 SonosKit tests: queue position, scroll anchor, group volume distribution, smart queue rules, browse walk order, artwork precedence, sidebar drop rules, locked-play override, synced-lyric interpolation, stale-URL parsing, media-server parsing and reachability, AI endpoint normalisation.
- A CI workflow runs the package tests, compiles the app target unsigned, and checks the localisation table for duplicate keys on every push.

### Localization

- Translations live in an Apple String Catalog, `Localizable.xcstrings` in the SonosKit package, instead of Swift dictionary literals: Xcode's catalog editor and XLIFF export work on it, Xcode compiles it into a strings table per language, and a duplicate key can no longer trap at launch. Lookup still follows the app's own language setting with an English fallback. `L10nCatalogTests` pins the contract on every test run: every key in all 13 languages, no duplicates, accessor and catalog keys in one-to-one agreement, and format placeholders matching English in every language.
- Every user-visible string is localised in all 13 languages: 1529 keys with no gaps, no duplicates, and no key without an accessor. Errors and status messages that had stayed English moved across in this release — the `AppError` banner text and its sanitised SOAP fault messages, the stale-data messages (speaker not responding, group changed, layout changed, nothing loaded, library not set up on the selected system), the reconnecting banner, the Plex sign-in and browse errors, the Sonos service sign-in errors, the Last.fm connect errors, the bug-report envelope and encryption errors, the update-check HTTP failure, the smart-queue card names, and the Apple Music live badge.
- App Intents strings (action titles, descriptions, parameter names, Siri phrases) stay in `Localizable.xcstrings` and `AppShortcuts.xcstrings` as App Intents requires, complete in all 13 languages; in-app strings stay in `L10n` so they follow the app's own language setting rather than the system's.
...
- Speakers can be added by IP address (Settings → System → Add speakers by address) for networks that block multicast outright, where no hop limit helps. The address is probed directly over HTTP, which crosses boundaries multicast cannot. One speaker is usually enough: the topology query on it returns the rest of the household. Seeded speakers are added to the event listener's accepted-peer set automatically.
